					...
					{technicians.map((tech, index) => (
						<FadeUp key={tech.id} delay={index * 150} class="group">
							<div class="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2 text-center border border-white/50 h-full flex flex-col">
								...
								{/* Bio Description */}
								<div class="mb-8 grow">
									<p class="font-montserrat text-sm text-neutral-content/80 leading-relaxed line-clamp-4">
										{getBio(tech)}
									</p>
								</div>
								...
								<Booking
									...
									classes="btn btn-outline btn-neutral rounded-full px-8 font-montserrat uppercase tracking-wider text-xs hover:bg-primary hover:text-white hover:border-primary transition-all duration-300 transform active:scale-95 w-full sm:w-auto"
								/>
							</div>
						</FadeUp>
					))}
