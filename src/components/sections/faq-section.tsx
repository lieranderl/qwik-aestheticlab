...
function collapseItemClass(isActive: boolean) {
	return `collapse collapse-arrow ${isActive ? "" : "border-b border-base-300"}`;
}
		...
		<section id="faq" class="scroll-mt-24 bg-base-200 py-16 md:py-24 lg:py-28">
			<div class="mx-auto grid w-full max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.72fr_1.28fr] lg:gap-16 lg:px-8">
				...
				<div class="card card-border w-full bg-base-100 shadow-sm">
					...
					<div class={collapseItemClass(true)}>
						<input
							type="radio"
						...
						<div
							...
							class="collapse-title min-h-14 py-5 pr-12 font-montserrat text-xl leading-snug font-medium text-base-content"
						>
							{bookingQuestion}
						</div>
						<div class="collapse-content pb-5">
							<div class="max-w-2xl font-montserrat text-sm leading-7 text-base-content">
								{bookingAnswerBefore}{" "}
								<Booking
									id="faq-book-btn"
									text={t("app.book.book_app@@Book Appointment")}
									location="372146"
									classes="btn btn-primary btn-xs min-h-8 align-baseline font-montserrat text-xs font-semibold uppercase tracking-wider"
					...
					{otherQuestions.map((item, index) => {
						...
						return (
							<div key={item.question} class={collapseItemClass(isLast)}>
								<input
									type="radio"
									name="landing-page-faq"
									checked={isLast}
								...
								<div class="collapse-content pb-5">
									<p
										class="max-w-2xl font-montserrat text-sm leading-7 text-base-content"
